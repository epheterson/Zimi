...
| F7 | Whole site, sqlite.org, Fast: at 14 s the job screen was replaced by the blank form (with the Recent list) for one poll, then the job came back at 26/200 pages. Seen once in a 2 s-frame run; not reproduced in a 1 s-frame run with the API watched at 0.3 s (no server flap). | Client: `_createIngest` turns `active=false, done=false` into "interrupted" and shows the form. Cause of the one occurrence unknown; the server never reported such a state in the watched run. | Reproduce with the API watcher before touching anything. | open |
