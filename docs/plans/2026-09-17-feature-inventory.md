...
Reddot, verified and made coy (2026-09-19, 23:00 to midnight). Eric: "Keep at it get Reddot verified rename the pill in create flow to Subreddit or actually let's be coy. You put in the url Reddit.com/r/whatever and we know what to do." and "Don't forget Ive at the end across zimi itself the app idea the entry points icons and apps themselves every bit of it". Done: no Subreddit tile; a reddit.com/r/<name> address under Web page or Site becomes the subreddit build on the server, the preview says Subreddit / What / Helper, Reddot's doors open Create with `https://www.reddit.com/r/` started in the field. A real r/Kiwix build (1,031 posts, 5,849 comments) through Zimi's own path found three ArcticZim problems, each handled on Zimi's side: the fetch never ends at the end of a subreddit (stall watch), the build leaves a forkserver holding the output pipe (the reader stops when the command has exited), and on macOS every build worker dies because ArcticZim takes every non-Linux system for Windows (a launcher Zimi writes into the sidecar patches that; a worker traceback fails the build instead of hanging). The reader was rewritten for ArcticZim's real minified pages; the fixture is now real-shaped. A subreddit ZIM is named by its subreddits (every ArcticZim ZIM is titled "ArcticZim"), files under Reddit, and the tile names them. Across Zimi: the apps row is labelled Apps like the sections around it; the server hint names all four apps.

Linux, every launch flow, and the pass across Zimi (2026-09-20). Eric: "Looks like someone is having an issue on Linux", then "I want full functionality on all platforms and launch flows and the Ive pass when you're done", and "Make sure we clean up after all work and investigations". Issue #81: reproduced the shipped 1.9.6 AppImage on the NAS in Docker under Xvfb. The Linux window had never opened on any machine: the bundle carried no WebKit2 typelib and GObject introspection saw only the bundle; with the host's typelib the window opened but the page stayed blank because the bundle's LD_LIBRARY_PATH reached WebKitGTK's helper processes; on 24.04 and Fedora the host's WebKitGTK failed against the bundle's own older GLib and gnutls. The snap had no display plugs. CI had only ever smoke-tested the headless server. And `zimi desktop` from pip never worked (the module was not in the wheel). Done: the desktop app moved into the package (`zimi.desktop`, shims kept), the bundle carries the WebKit2 typelibs and nothing else of the system (Python and the wheels only), the host's platform comes first, WebKit is pinned to the ABI the host has, the helper processes get the host's library path, WebKitGTK's GPU escape hatches are set, a machine with no WebKitGTK or Qt runs Zimi in the system browser (`--browser` asks for it anywhere), `zimi[desktop]` extra, snap gets the GNOME extension and display plugs, CI opens the window under Xvfb and runs the browser mode. Across Zimi: the apps row is labelled, the catalog's category marks are drawn in Zimi's line rather than emoji, the Manage box says one thing. Verified across Ubuntu 22.04 (WebKit 4.0 only), 24.04, Debian 12, Fedora 41 and a machine with no WebKit, on the NAS. Known limit: Debian 11 / q4os 5 (glibc 2.31) cannot run a bundle built on 22.04 at all.
