...
- **"This video cannot be played here."** — the file is in the ZIM but this browser cannot decode it. The link under the message opens the video's own page, which may carry a decoder of its own (TED's pages do). A ZIM Zimi makes from a video site takes H.264 first for this reason; one made before 1.10 may carry AV1, which no iPhone before the 15 Pro decodes.
