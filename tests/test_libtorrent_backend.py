...
class TestResume:
    ...
    def test_remove_after_stop_keeps_the_resume_file(self, backend, tmp_path):
        """stop() writes every torrent's resume data and clears the handles.
        The download poll, still running for a moment, then sees its handle
        gone and calls remove(): that must not delete what stop() just wrote,
        or the next start re-verifies the whole partial from zero."""
        tid = backend.add_torrent(
            str(tmp_path / "keep.torrent"), dest_dir=str(tmp_path / "staging")
        )
        backend._handles[tid].save_resume_data()
