...
- Zimi Tube. A Tube tile sits with the sources whenever a video ZIM is installed and opens one feed of every video in the library: Kiwix's TED and TED-Ed, YouTube channels and Khan Academy, and the ZIMs `zimi create` makes from a playlist. Thumbnail, title, speaker and which ZIM on every card; the search bar filters the feed; a card opens the video's own page, and Back returns to the feed. `zimi create <video URL>` now writes a videos.json into its ZIMs.
