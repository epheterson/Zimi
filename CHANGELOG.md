...
**Tell us about the sites that fail.** Every capture engine has been driven through twenty-six sites and set beside the ZIMs Kiwix ships for the same pages, and each defect found became a fix with a test. The web is bigger than that. When a site captures badly, opens naked, misses its pictures, or refuses the crawler, open an issue with the address and what you saw; the failed card on the create page has a link that fills one in for you. A site that fails is a fixture we do not have yet.
