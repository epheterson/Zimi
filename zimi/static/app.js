...
async function setLanguage(lang) {
  ...
  if (readerOpen) {
    ...
    switchManageTab(manageTab || 'installed');
...
function _langBannerDismiss(lang) {
  sessionStorage.setItem('zimi_lang_banner_' + lang, '0');
  var banner = document.getElementById('lang-banner');
  if (banner) { banner.remove(); _adjustBodyPadding(); }
}
  ...
  fetch('/article-languages?zim=' + encodeURIComponent(currentArticle.zim) + '&path=' + encodeURIComponent(currentArticle.path))
    ...
    .then(function(data) {
      if (_articleLangKey === key) {
        ...
        // Re-render dropdown if it's open (data arrived after user opened it)
        var dd = document.getElementById('lang-dropdown');
        if (dd && dd.classList.contains('visible')) _renderLangDropdown();
      }
    })
    .catch(function() {});
}
...
async function openSettings() {
  ...
  document.getElementById('settings-data-browse-btn').style.display = '';
  ...
  try {
    ...
    document.getElementById('settings-data-dir').value = cfg.data_dir || '';
    portInput.value = cfg.port || 8899;
    _settingsOriginal = { zim_dir: cfg.zim_dir, data_dir: cfg.data_dir || '', port: cfg.port };
  } catch(e) {}
...
async function settingsChooseDataFolder() {
  if (!IS_DESKTOP) return;
...
async function settingsSave() {
  ...
  if (IS_DESKTOP) {
    ...
    const dataDirVal = document.getElementById('settings-data-dir').value || '';
    if (dataDirVal !== _settingsOriginal.data_dir) updates.data_dir = dataDirVal;
    ...
    try {
      ...
      if (needsRestart) {
        msgEl.textContent = t('restarting');
