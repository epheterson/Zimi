...
def _qid_cache_store(zim_name, path, qid):
    ...
    try:
        ...
        log.debug("Q-ID cache store failed for %s/%s: %s", zim_name, path, e)
...
def get_article_languages(zim_name, article_path):
    ...
    for lang, candidates in lang_candidates.items():
        ...
        for cand_name, _ in candidates:
            ...
            for search_fn in [
                lambda: _srv.suggest_search_zim(cand_archive, human_title, 5),
                lambda: _srv.search_zim(cand_archive, human_title, limit=3, snippets=False),
            ]:
                try:
                    results = search_fn()
                    for r in results:
                    ...
                    log.debug("Language match search failed for %s in %s: %s", human_title, cand_name, e)
                    pass
                if found:
                    break
            if found:
